...
while(RealTime<H)
{
	...
	{	
		...
		NT=NNeighborBlockT(Psitemp)[nOmega]
		indGamma=NT
		IndexGamma<-c(IndexGamma,indGamma)
		Mbelief<-Gamma[,indGamma]
		Mprojprobs<-c(sum(Mbelief[1:nm0]),sum(Mbelief[(nm0+1):(nm0+nm1)]),sum(Mbelief[(nm0+nm1+1):(nm0+nm1+nm2)]))
		D2=c(D2,distblock(c(Mbelief,Mprojprobs),Psitemp))
		Dist=c(Dist,sqrt(sum((Psitemp-Gamma[,NT])^2)))
		Modes<-rbind(Modes,Mprojprobs)
